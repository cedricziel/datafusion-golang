...
## Requirements
### Requirement: Open a local JSON Lines file with a caller-supplied schema

The package SHALL let a Go program construct a table provider from the path to a local JSON Lines file and an `*arrow.Schema` supplied by the caller. Schema inference is not supported — the caller MUST always supply the schema. Construction SHALL fail with a Go error if the file does not exist, is not readable, or if its first JSON object cannot be decoded against the supplied schema, rather than deferring the failure to the first scan.
