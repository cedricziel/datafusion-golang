...
// the provider itself is untouched by that change, but the engine's
// error message should be a clear "does not support INSERT" rather than
...
func TestInsertIntoFailsCleanly(t *testing.T) {
	...
	if err == nil {
		t.Fatalf("expected INSERT against a read-only parquet provider to fail")
	}
